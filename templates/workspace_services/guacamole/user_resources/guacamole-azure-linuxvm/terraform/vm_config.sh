...
if [ "${CONDA_CONFIG}" -eq 1 ]; then
  ...
  conda config --add channels "${NEXUS_PROXY_URL}"/repository/conda-forge/main/  --system
  conda config --add channels "${NEXUS_PROXY_URL}"/repository/conda/main/  --system
  conda config --remove channels defaults --system
  conda config --set channel_alias "${NEXUS_PROXY_URL}"/repository/conda-forge/  --system
fi
