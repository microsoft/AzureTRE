# Optional wait to allow the NIC to detach before it is destroyed. Used by the
# airlock review VMs which are frequently created/destroyed.
resource "time_sleep" "wait_for_nic_destroy" {
  count = var.enable_nic_destroy_wait ? 1 : 0
...
resource "azurerm_windows_virtual_machine" "windowsvm" {
  ...
  depends_on = [time_sleep.wait_for_nic_destroy]
}
...
# credentials. Skipped when owner_id is empty (e.g. airlock review VMs).
resource "azurerm_role_assignment" "vm_password_secret_reader" {
  count                = var.owner_id != "" ? 1 : 0
  scope                = azurerm_key_vault_secret.windowsvm_password.resource_versionless_id
