...
deploy-shared-service:
	...
	&& . ${MAKEFILE_DIR}/devops/scripts/get_access_token.sh \
	&& cd ${DIR} \
	&& ${MAKEFILE_DIR}/devops/scripts/deploy_shared_service.sh --insecure --tre_url "$${TRE_URL:-https://$${TRE_ID}.$${LOCATION}.cloudapp.azure.com}" --properties "$${PROPS}"
...
nexus-install:
	$(MAKE) bundle-build DIR=${MAKEFILE_DIR}/templates/shared_services/certs/ \
	&& $(MAKE) bundle-publish DIR=${MAKEFILE_DIR}/templates/shared_services/certs/ \
	&& $(MAKE) bundle-register DIR="${MAKEFILE_DIR}/templates/shared_services/certs" BUNDLE_TYPE=shared_service \
	&& $(MAKE) deploy-shared-service DIR=${MAKEFILE_DIR}/templates/shared_services/certs/ BUNDLE_TYPE=shared_service PROPS='"domain_prefix": "nexus","cert_name": "nexus-ssl"' \
	&& $(MAKE) bundle-custom-action DIR=${MAKEFILE_DIR}/templates/shared_services/certs/ ACTION=generate \
	&& $(MAKE) bundle-build DIR=${MAKEFILE_DIR}/templates/shared_services/sonatype-nexus-vm/ \
	&& $(MAKE) bundle-publish DIR=${MAKEFILE_DIR}/templates/shared_services/sonatype-nexus-vm/ \
	&& $(MAKE) bundle-register DIR="${MAKEFILE_DIR}/templates/shared_services/sonatype-nexus-vm" BUNDLE_TYPE=shared_service \
	&& $(MAKE) deploy-shared-service DIR=${MAKEFILE_DIR}/templates/shared_services/sonatype-nexus-vm/ BUNDLE_TYPE=shared_service
