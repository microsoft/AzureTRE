...
from resources.workspace import assign_airlock_manager_role, get_workspace_auth_details
from airlock import strings as airlock_strings
from conftest import get_workspace_owner_token
...
async def submit_airlock_import_request(workspace_path: str, workspace_owner_token: str, verify: bool):
    ...
    while not blob_uploaded:
        ...
        try:
            ...
            i += 1
            LOGGER.info(f"Waiting {wait_time}s for container creation. If it does not create check airlock processor.")
...
@pytest.mark.airlock
async def test_airlock_review_vm_flow(setup_airlock_test_workspaces_and_guacamole_service, verify):

    base_workspace_path, base_workspace_id, import_review_workspace_path, import_review_workspace_id, guacamole_workspace_service_path, guacamole_workspace_service_id = setup_airlock_test_workspaces_and_guacamole_service
    ...
    # Preparation: Get the workspace owner token
        ...
        "properties": {
            ...
            "airlock_review_config": {
                "import": {
                    ...
                    "import_vm_workspace_service_id": guacamole_workspace_service_id,
    ...
    workspace = await get_resource(f"/api{base_workspace_path}", workspace_owner_token, verify)
    admin_token = await get_admin_token(verify)

    await post_resource(
        payload=patch_payload,
        endpoint=f"/api{base_workspace_path}",
    ...
    # IMPORT FLOW
    # Submit the request
    request_id, _ = await submit_airlock_import_request(base_workspace_path, workspace_owner_token, verify)
    ...
    user_resource_path, user_resource_id = await post_resource(
        payload={},
        endpoint=f"/api{base_workspace_path}/requests/{request_id}/review-user-resource",
    ...
    request_result = await post_request(payload, f'/api{base_workspace_path}/requests/{request_id}/review', workspace_owner_token, verify, 200)
    assert request_result["airlockRequest"]["reviews"][0]["decisionExplanation"] == "the reason why this request was approved/rejected"

    await wait_for_status(airlock_strings.APPROVED_STATUS, workspace_owner_token, base_workspace_path, request_id, verify)
...
async def test_airlock_flow(setup_test_base_workspace, verify) -> None:
    ...
    request_result = await get_request(f'/api{base_workspace_path}/requests/{request_id}/link', workspace_owner_token, verify, 200)
    container_url = request_result["containerUrl"]
    ...
    request_result = await post_request(payload, f'/api{base_workspace_path}/requests', workspace_owner_token, verify, 201)
