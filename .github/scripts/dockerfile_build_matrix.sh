...
#   safe_name   The label with "/" replaced, for use in artifact names
...
#   CHANGED_FILES     Changed Dockerfile, porter.yaml and .devcontainer paths,
#                     space separated (pull requests only)
...
# A target is selected when one of the changed files is its Dockerfile, its
# porter.yaml, or, for the dev container, any file under .devcontainer/.
is_selected() {
  local dir="$1"
  local dockerfile="$2"
...
for entry in "${plain_targets[@]}"; do
  ...
  if is_selected "$dir" "$dockerfile"; then
    name="$(label "$dir")"
    targets+=("$(jq -c -n \
      --arg name "$name" \
      --arg safe_name "${name//\//_}" \
