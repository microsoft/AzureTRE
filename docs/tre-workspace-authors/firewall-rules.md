...
| `description` | Human-readable description (note: not used in Azure - deprecated for network rules) | "Azure Machine Learning rules" |
...
| `description` | Human-readable description (must be non-empty string if provided) | "AzureML rules" |
...
1. **Use descriptive names and descriptions** for rule collections and individual rules. Note that descriptions are only supported for application rules (rule_collections), not network rules (network_rule_collections), and must be non-empty strings.
