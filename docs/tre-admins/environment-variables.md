...
| `AUTO_WORKSPACE_GROUP_CREATION` | Set to `true` in `config.sample.yaml` (Terraform default remains `false`). Setting this to `true` grants the `Group.Create` permission to the *Application Admin* identity. This identity can then create security groups aligned to each application role. Microsoft Entra ID licensing implications need to be considered, as group assignment is a premium feature. |
| `AUTO_GRANT_WORKSPACE_CONSENT` | Set to `true` in `config.sample.yaml` (Terraform default remains `false`). Setting this to `true` will remove the need for users to manually grant consent when creating new workspaces. The identity will be granted `Application.ReadWrite.All` and `DelegatedPermissionGrant.ReadWrite.All` permissions. |
| `USER_MANAGEMENT_ENABLED` | Set to `true` in `config.sample.yaml`. If set to `true`, TRE Admins will be able to assign and de-assign users to workspaces via the UI (Requires Entra ID groups to be enabled on the workspace and the workspace template version to be 2.2.0 or greater). |
