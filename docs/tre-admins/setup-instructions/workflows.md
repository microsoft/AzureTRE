...
Deployment is done using the `/.github\workflows\deploy_tre.yml` workflow. This method is also used to deploy the dev/test environment for the original AzureTRE repository.
...
| `SWAGGER_UI_CLIENT_ID` | The application (client) ID of the TRE Swagger UI Service Principal. |
| `API_CLIENT_ID` | The application (client) ID of the TRE API Service Principal. |
