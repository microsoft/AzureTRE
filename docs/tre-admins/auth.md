...
| AUTO_WORKSPACE_GROUP_CREATION | Set to `true` in `config.sample.yaml`. Setting this to `true` grants the `Group.Create` permission to the *Application Admin* identity. This identity can then create security groups aligned to each application role. Microsoft Entra ID licensing implications need to be considered, as group assignment is a premium feature. [You can read more about group assignment here](https://docs.microsoft.com/en-us/azure/architecture/multitenant-identity/app-roles#roles-using-azure-ad-app-roles). |
| AUTO_GRANT_WORKSPACE_CONSENT | Set to `true` in `config.sample.yaml`. Setting this to `true` will remove the need for users to manually grant consent when creating new workspaces. The identity will be granted `Application.ReadWrite.All` and `DelegatedPermissionGrant.ReadWrite.All` permissions. |
