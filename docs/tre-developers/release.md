...
3. Update `CHANGELOG.md` in a PR with the following:
   ...
   2. Create a new section for the next-unreleaed version so that future changes will be placed there.
...
5. Create GitHub Release in `Pre Release` state.
   ...
   2. Click on `Choose a tag` and type a new one for you version. It should be in the form of `v0.9.2` - note the "v" in the begining.
