...
3. **In-Review**: The request is ready to be reviewed. This state can be reached directly from the Submitted state or after going through a successful security scan (found clean).
...
Once this manual review is executed, the Airlock Managers can proactively approve or reject the airlock request.
