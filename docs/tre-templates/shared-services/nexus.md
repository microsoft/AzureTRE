...
| Microsoft Apt | apt | [https://packages.microsoft.com] | `https://nexus-{TRE_ID}.{LOCATION}.cloudapp.azure.com/repository/microsoft-apt` | Provide access to Microsoft Apt packages (Ubuntu 22.04/jammy). The remoteUrl is the host root; per-product paths and the `jammy` distribution are encoded in client sources. |
