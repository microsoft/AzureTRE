...
@patch("api.routes.workspaces.WorkspaceRepository.save_workspace")
@patch("api.routes.workspaces.WorkspaceRepository.create_workspace_item")
async def test_workspaces_post_calls_db_and_service_bus(create_workspace_item_mock, save_workspace_mock, send_resource_request_message_mock, service_bus_client_mock, app: FastAPI, client: AsyncClient):
    service_bus_client_mock().get_queue_sender().send_messages = AsyncMock()
    workspace_id = "000000d3-82da-4bfc-b6e9-9a7853ef753e"
    create_workspace_item_mock.return_value = create_sample_workspace_object(workspace_id)
    input_data = create_sample_workspace_input_data()
...
@patch("api.routes.workspaces.WorkspaceRepository.save_workspace")
@patch("api.routes.workspaces.WorkspaceRepository.create_workspace_item")
async def test_workspaces_post_returns_202_on_successful_create(create_workspace_item_mock, save_workspace_mock, send_resource_request_message_mock, service_bus_client_mock, app: FastAPI, client: AsyncClient):
    service_bus_client_mock().get_queue_sender().send_messages = AsyncMock()
    workspace_id = "000000d3-82da-4bfc-b6e9-9a7853ef753e"
    create_workspace_item_mock.return_value = create_sample_workspace_object(workspace_id)
    input_data = create_sample_workspace_input_data()
