...
* Fix clean validation workflow failing due to azure-firewall extension checksum mismatch by replacing `az network firewall` extension commands with `az rest` ARM REST API calls, removing the extension dependency entirely ([#4971](https://github.com/microsoft/AzureTRE/issues/4971))
