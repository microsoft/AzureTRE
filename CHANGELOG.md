...
BUG FIXES:
* Add `bx-msedge.net.` and `vsassets.io.` to the default DNS allowlist and `*.vsassets.io` to the Nexus firewall allowlist so Nexus can proxy VS Code Marketplace extensions when DNS security policy is enabled. (`core` 0.16.18, `tre-shared-service-sonatype-nexus` 3.10.3) ([#5065](https://github.com/microsoft/AzureTRE/issues/5065))
