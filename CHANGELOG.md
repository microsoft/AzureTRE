...
BUG FIXES:
* Fix airlock Event Grid 503 when a user token omits both `email` and `preferred_username` claims: the derived `AuthenticatedUser` email now defaults to an empty string so the airlock notification payload (which requires a string email) no longer fails validation. ([#4989](https://github.com/microsoft/AzureTRE/pull/4989))
