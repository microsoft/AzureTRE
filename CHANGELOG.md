...
BUG FIXES:
* Fix Nexus apt proxy for `microsoft-apt`: split into `microsoft-apt-bionic` and `microsoft-apt-jammy` per-distribution proxies behind a `microsoft-apt` group (matching the pattern used for `ubuntu`, `ubuntu-security`, and `docker`), so both bionic and jammy VMs can resolve Microsoft packages. Fix upgrade path for existing Nexus instances: delete any conflicting non-group repo with the same name as a new apt group before creating the group, so the group+member migration succeeds without manual intervention. (`sonatype-nexus` 3.10.2) ([#4992](https://github.com/microsoft/AzureTRE/issues/4992))
* Fix Guacamole Linux VM bootstrap failures on Ubuntu 22.04 (jammy): rework `ubuntu`, `ubuntu-security`, and `docker` Nexus apt repos into per-distribution proxy members (`-bionic`, `-jammy`) behind stable apt group repos, so clients on any supported Ubuntu release use the same group URL without reconfiguration. Update `microsoft-apt` distribution to `jammy`. (`sonatype-nexus` 3.10.1) ([#4992](https://github.com/microsoft/AzureTRE/issues/4992))
