...
BUG FIXES:
* Fix Guacamole Linux VM bootstrap failures on Ubuntu 22.04 (jammy): rework `ubuntu`, `ubuntu-security`, `docker`, and `microsoft-apt` Nexus apt repos into per-distribution proxy members (`-bionic`, `-jammy`) behind stable apt group repos, so clients on any supported Ubuntu release use the same group URL without reconfiguration. Fix upgrade path for existing Nexus instances by deleting conflicting non-group repos before creating groups. Add cloud-init wait to Guacamole Linux VM so deployment reports failure if cloud-init errors. (`sonatype-nexus` 3.10.2, `tre-service-guacamole-linuxvm` 1.4.4) ([#4992](https://github.com/microsoft/AzureTRE/issues/4992), [#4540](https://github.com/microsoft/AzureTRE/issues/4540))
