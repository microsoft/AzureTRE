...
ENHANCEMENTS:
* Make Airlock v2 (consolidated metadata-based storage) the default for new airlock requests, retaining legacy per-stage storage behind `enable_legacy_airlock` (defaults to `true`; the sample config sets `false`). Pre-existing workspaces and in-flight requests are backfilled with `airlock_version=1` by a DB migration so they keep routing to legacy storage; each workspace resolves the shared global airlock account to its own private endpoint via a workspace-scoped DNS zone and signs SAS with a per-workspace airlock signer identity (so multiple workspaces no longer collide on the shared-account role assignment and a leaked SAS cannot be replayed from another workspace); changing a workspace's `airlock_version` is blocked while it has in-flight requests, and when `enable_legacy_airlock=false` the API blocks creating a v1 workspace (a `POST /migrations` run stamps `airlock_version=1` on pre-v2 workspaces/requests so they keep routing to legacy storage); airlock v2 requires automatic app registration, so manual-auth workspaces default to (and are restricted to) v1; the import-review workspace provisions connectivity to the legacy `stalimip` account when `enable_legacy_airlock` is set (now auto-propagated from the core setting so UI-created review workspaces match the deployment), so review VMs can still reach v1 in-progress data; and the review-VM SAS, legacy import account naming, malware scan verdict handling on submit, cross-subscription workspace planning, blob-created approval completion handling, and the core private-link ABAC condition are corrected. See [Legacy Airlock & migration](docs/azure-tre-overview/airlock.md#legacy-airlock). (`core` 0.18.5, `api` 0.27.6, `airlock-processor` 0.8.18, `tre-workspace-base` 3.10.1, `tre-workspace-airlock-import-review` 1.6.1) ([#5048](https://github.com/microsoft/AzureTRE/pull/5048))
