...
* Refactor the Guacamole Windows VM and airlock import/export review VMs to share a single Terraform VM module (`terraform/vm`) instead of duplicated/symlinked `.tf` files. Bundles are bumped to a new major version (`tre-service-guacamole-windowsvm` 2.0.0, `tre-service-guacamole-import-reviewvm` and `tre-service-guacamole-export-reviewvm` 1.0.0); existing VM user resources cannot be upgraded in place and must be redeployed. ([#4981](https://github.com/microsoft/AzureTRE/pull/4981))

ENHANCEMENTS:
* Install data science and Azure tooling on the Guacamole Windows VM and airlock review VMs via the shared `vm_config.ps1`, pulled through the Nexus proxy to replace the removed DSVM images: Azure CLI, Visual Studio Code, Azure Storage Explorer, Python (Miniforge) with JupyterLab, R, RStudio Desktop, PyCharm Community and Git. Desktop shortcuts are created for the installed GUI tools. (`tre-service-guacamole-windowsvm` 2.1.0, `tre-service-guacamole-import-reviewvm` and `tre-service-guacamole-export-reviewvm` 1.1.0) ([#4981](https://github.com/microsoft/AzureTRE/pull/4981))
