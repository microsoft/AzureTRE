...
export const SecuredByRole: React.FunctionComponent<SecuredByRoleProps> = (props: SecuredByRoleProps) => {
  ...
  useEffect(() => {
    const getWorkspaceRoles = async () => {
      if (!workspaceCtx.workspace.id && props.workspaceId !== "") {
        ...
        let workspaceAuth = (await apiCall(`${ApiEndpoint.Workspaces}/${props.workspaceId}/scopeid`, HttpMethod.Get))
          .workspaceAuth;
        if (workspaceAuth) {
          await apiCall(
  ...
  return workspaceRoles?.some((x) => props.allowedWorkspaceRoles?.includes(x)) ||
