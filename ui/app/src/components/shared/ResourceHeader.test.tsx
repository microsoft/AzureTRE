...
vi.mock("./PowerStateBadge", () => {
  const PowerStateBadge = ({ state }: any) => <div data-testid="power-state-badge">Power: {state}</div>;
...
describe("ResourceHeader Component", () => {
  it("renders resource display name", () => {
    render(<ResourceHeader resource={mockResource} latestUpdate={mockLatestUpdate} />);
  ...
  it("does not render power state badge when not available", () => {
    ...
    render(<ResourceHeader resource={resourceWithoutPowerState} latestUpdate={mockLatestUpdate} />);
  ...
  it("renders status badge with operation status when available", () => {
    ...
    render(<ResourceHeader resource={mockResource} latestUpdate={latestUpdateWithOperation} />);
  ...
  it("does not render context menu when readonly", () => {
    render(<ResourceHeader resource={mockResource} latestUpdate={mockLatestUpdate} readonly={true} />);
  ...
  it("renders progress indicator when resource is locked", () => {
    ...
    render(<ResourceHeader resource={mockResource} latestUpdate={lockedUpdate} />);
  ...
  it("passes component action to context menu", () => {
    ...
    render(<ResourceHeader resource={mockResource} latestUpdate={updateWithAction} />);
  ...
  it("renders nothing when resource has no id", () => {
    ...
    const { container } = render(<ResourceHeader resource={resourceWithoutId} latestUpdate={mockLatestUpdate} />);
  ...
  it("renders with proper layout structure", () => {
    ...
    const headerStack = stacks.find((stack) => stack.getAttribute("data-horizontal") === "true");
  ...
  it("aligns status badge to center", () => {
    ...
    const stackItems = screen.getAllByTestId("stack-item");
    const statusItem = stackItems.find((item) => item.getAttribute("data-align") === "center");
