      ...
      {props.operation.dismiss ? (
        ...
        <li className="tre-notification-item">
          ...
          <ProgressIndicator
            barHeight={4}
            percentComplete={
              awaitingStates.includes(props.operation.status)
                ? 0
                : completedStates.includes(props.operation.status)
                  ? 100
                  : undefined
            }
            label={
          ...
          <Stack horizontal style={{ marginTop: "10px" }}>
            <Stack.Item grow={5}>
              ...
              !(
                ...
                >
                  {isExpanded ? (
                    <Icon iconName="ChevronUp" aria-label="Expand Steps" />
                  ) : (
                    <Icon iconName="ChevronDown" aria-label="Collapse Steps" />
                  )}
                </FluentLink>
              ) : (
                " "
                ...
                {props.operation.steps &&
                  props.operation.steps.map((s: OperationStep, i: number) => {
                    return (
                      <li key={i}>
                        <Icon
                          iconName={getIconAndColourForStatus(s.status)[0]}
                          style={{
                            color: getIconAndColourForStatus(s.status)[1],
                            position: "relative",
                            top: "2px",
                            marginRight: "10px",
                          }}
