...
@click.command(name="new", help="Register a new workspace template")
...
@output_option()
@query_option()
def workspace_templates_create(definition, definition_file, output_format, query):
    log = logging.getLogger(__name__)
    ...
    response = client.call_api(log, 'POST', '/api/workspace-templates', json_data=definition_dict)
...
workspace_templates.add_command(workspace_templates_create)
