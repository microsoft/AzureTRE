...
def _handle_v2_blob_created(json_body, topic, request_id, stepResultEvent, dataDeletionEvent):
    ...
    In v2, StatusChangedQueueTrigger owns stage-transition completion: same-account
    transitions update container metadata inline, and cross-account copies are performed
    synchronously (StatusChangedQueueTrigger waits for the copy to finish and emits the
    StepResult itself). Relying on this BlobCreated event to signal completion is racy —
    it can fire on empty-container/metadata-only writes before the copy finishes, and the
    workspace-global account is default-deny which can block metadata reads. We therefore
    intentionally do not emit any StepResult here; these events are logged and ignored.
    """
    logging.info(
        f"V2 BlobCreated event for request {request_id} on topic '{topic}' ignored - "
        "stage completion is handled synchronously by StatusChangedQueueTrigger.")
