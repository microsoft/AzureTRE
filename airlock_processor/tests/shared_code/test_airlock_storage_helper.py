...
class TestGetStorageAccountNameForRequestConsolidated:
    ...
    class TestImportRequests:
        ...
        def test_import_submitted_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_SUBMITTED)
            assert account == "stalairlocktre123"

        def test_import_in_review_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_IN_REVIEW)
            assert account == "stalairlocktre123"

        def test_import_approved_uses_workspace_global_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_APPROVED)
            assert account == "stalairlockgtre123"

        def test_import_approval_in_progress_uses_workspace_global_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_APPROVAL_INPROGRESS)
            assert account == "stalairlockgtre123"

        def test_import_rejected_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_REJECTED)
            assert account == "stalairlocktre123"

        def test_import_rejection_in_progress_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_REJECTION_INPROGRESS)
            assert account == "stalairlocktre123"

        def test_import_blocked_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_BLOCKED_BY_SCAN)
            assert account == "stalairlocktre123"

        def test_import_blocking_in_progress_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.IMPORT_TYPE, constants.STAGE_BLOCKING_INPROGRESS)
            assert account == "stalairlocktre123"
    ...
    class TestExportRequests:
        ...
        def test_export_submitted_uses_workspace_global_storage(self):
            account = get_storage_account_name_for_request(constants.EXPORT_TYPE, constants.STAGE_SUBMITTED)
            assert account == "stalairlockgtre123"

        def test_export_approved_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.EXPORT_TYPE, constants.STAGE_APPROVED)
            assert account == "stalairlocktre123"

        def test_export_approval_in_progress_uses_core_storage(self):
            account = get_storage_account_name_for_request(constants.EXPORT_TYPE, constants.STAGE_APPROVAL_INPROGRESS)
            assert account == "stalairlocktre123"

        def test_export_rejected_uses_workspace_global_storage(self):
            account = get_storage_account_name_for_request(constants.EXPORT_TYPE, constants.STAGE_REJECTED)
            assert account == "stalairlockgtre123"
