...
jq --arg rootClientId "${SWAGGER_UI_CLIENT_ID}" \
  ...
  --arg uiProductName "${UI_PRODUCT_NAME:-}" \
  --arg uiFooterText "${UI_FOOTER_TEXT:-}" \
  '.rootClientId = $rootClientId | .rootTenantId = $rootTenantId | .treApplicationId = $treApplicationId | .treUrl = $treUrl | .treId = $treId | .version = $version | .activeDirectoryUri = $activeDirectoryUri | .userManagementEnabled = $userManagementEnabled | .uiProductName = $uiProductName | .uiFooterText = $uiFooterText' ./src/config.source.json > ./src/config.json
