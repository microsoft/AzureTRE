...
then
  ...
  if [ ! -z "${TEST_AUTOMATION_ACCOUNT_CLIENT_ID:-}" ] && [ ! -z "${TEST_AUTOMATION_ACCOUNT_CLIENT_SECRET:-}" ] && [ ! -z "${AAD_TENANT_ID:-}" ] && [ ! -z "${API_CLIENT_ID:-}" ]
  then
    # Use client credentials flow with TEST_AUTOMATION_ACCOUNT_CLIENT_ID/SECRET
    echo "Using TEST_AUTOMATION_ACCOUNT_CLIENT_ID to get token via client credential flow"
    token_response=$(curl -X POST -H 'Content-Type: application/x-www-form-urlencoded' \
      ...
      -d "client_id=${TEST_AUTOMATION_ACCOUNT_CLIENT_ID}"   \
      -d 'grant_type=client_credentials'   \
      -d "scope=api://${API_CLIENT_ID}/.default"   \
      -d "client_secret=${TEST_AUTOMATION_ACCOUNT_CLIENT_SECRET}")
