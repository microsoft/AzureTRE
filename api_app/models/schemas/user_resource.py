...
class UserResourcesInList(BaseModel):
    userResources: List[UserResource] = Field(default_factory=list, title="User resources")
    model_config = ConfigDict(json_schema_extra={
        "example": {
            "userResources": [
...
class UserResourceInCreate(BaseModel):
    ...
    properties: dict = Field(default_factory=dict, title="User resource parameters", description="Values for the parameters required by the user resource specification")
    model_config = ConfigDict(json_schema_extra={
        "example": {
