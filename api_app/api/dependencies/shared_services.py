...
def get_shared_service_by_id(shared_service_id: str, shared_services_repo) -> SharedService:
    try:
...
async def get_shared_service_by_id_from_path(shared_service_id: str = Path(...), shared_service_repo=Depends(get_repository(SharedServiceRepository))) -> SharedService:
    return get_shared_service_by_id(shared_service_id, shared_service_repo)


async def get_operation_by_id_from_path(operation_id: str = Path(...), operations_repo=Depends(get_repository(OperationRepository))) -> Operation:
    try:
