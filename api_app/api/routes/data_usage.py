...
get_storage_account_limit = APIRouter(dependencies=[Depends(get_current_workspace_owner_or_tre_user_or_tre_admin)])
...
@get_storage_account_limit.get("/storage_account_limit", response_model=MHRAWorkspaceDataUsage,
                       status_code=status.HTTP_200_OK,
                       name=strings.API_GET_WORKSPACE_DATA_USAGE_CLIENTS,
                       dependencies=[Depends(get_current_workspace_owner_or_tre_user_or_tre_admin)],
                       responses=get_workspace_data_usage_responses())
