...
async def get_user_resource_templates_for_service_template(service_template_name: str, template_repo=Depends(ResourceTemplateRepository.get_repository())) -> ResourceTemplateInformationInList:
...
async def get_user_resource_template(service_template_name: str, user_resource_template_name: str, is_update: bool = False, version: Optional[str] = None, template_repo=Depends(ResourceTemplateRepository.get_repository())) -> UserResourceTemplateInResponse:
...
async def register_user_resource_template(template_input: UserResourceTemplateInCreate, template_repo=Depends(ResourceTemplateRepository.get_repository()), workspace_service_template=Depends(get_workspace_service_template_by_name_from_path)) -> UserResourceTemplateInResponse:
    try:
