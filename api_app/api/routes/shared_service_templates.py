...
@shared_service_templates_core_router.get("/shared-service-templates/{shared_service_template_name}/{version}", response_model=SharedServiceTemplateInResponse, response_model_exclude_none=True, name=strings.API_GET_SHARED_SERVICE_TEMPLATE_BY_NAME_AND_VERSION, dependencies=[Depends(get_current_tre_user_or_tre_admin)])
async def get_shared_service_template_by_name_and_version(shared_service_template_name: str, version, is_update: bool = False, template_repo=Depends(get_repository(ResourceTemplateRepository))) -> SharedServiceTemplateInResponse:
    try:
        template = get_template_by_name_and_version(shared_service_template_name, version, template_repo, ResourceType.SharedService, is_update=is_update)
        return parse_obj_as(SharedServiceTemplateInResponse, template)
    except EntityDoesNotExist:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=strings.SHARED_SERVICE_TEMPLATE_DOES_NOT_EXIST)
