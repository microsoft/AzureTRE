...
from api.routes.shared_service_templates import get_current_template_by_name
...
@shared_service_templates_core_router.get("/shared-service-templates/{service_template_name}", response_model=SharedServiceTemplateInResponse, response_model_exclude_none=True, name=strings.API_GET_SHARED_SERVICE_TEMPLATE_BY_NAME, dependencies=[Depends(get_current_tre_user_or_tre_admin)])
async def get_current_shared_service_template_by_name(service_template_name: str, template_repo=Depends(get_repository(ResourceTemplateRepository))) -> SharedServiceTemplateInResponse:
    template = get_current_template_by_name(service_template_name, template_repo, ResourceType.SharedService)
...
async def register_shared_service_template(template_input: SharedServiceTemplateInCreate, template_repo=Depends(get_repository(ResourceTemplateRepository))) -> ResourceTemplateInResponse:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=strings.SHARED_TEMPLATE_VERSION_EXISTS)
