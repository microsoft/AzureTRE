...
async def exit_and_reject(airlock_request=Depends(get_airlock_request_by_id_from_path),
    ...
    if airlock_request.type == AirlockRequestType.Import:
        raise HTTPException(status_code=status_code.HTTP_400_BAD_REQUEST,
                            detail="Endpoint not available for Import Airlock requests.")
    try:
        rejected_airlock_request = await exit_and_reject_airlock_request(airlock_request, airlock_request_repo, user, workspace)
...
@airlock_workspace_router.post("/workspaces/{workspace_id}/requests/{airlock_request_id}/contact", status_code=status_code.HTTP_200_OK,
                               response_model=AirlockRequestWithAllowedUserActions, name=strings.API_CONTACT_TEAM_FORM,
                               dependencies=[Depends(get_current_workspace_owner_or_researcher_user), Depends(get_workspace_by_id_from_path)])
...
async def review_statistics_statements(airlock_request_statistics_statements_input: AirlockRequestStatisticsStatements,
    ...
    if airlock_request.type == AirlockRequestType.Import:
        raise HTTPException(status_code=status_code.HTTP_400_BAD_REQUEST,
                            detail="Endpoint not available for Import Airlock requests.")
    try:
        cheked_airlock_request = await save_and_check_statistics_statements(airlock_request, airlock_request_repo,
                                                                        airlock_request_statistics_statements_input)
        return AirlockRequestWithAllowedUserActions(airlockRequest=cheked_airlock_request)
    except (ValidationError, ValueError) as e:
...
@airlock_workspace_router.post("/workspaces/{workspace_id}/requests/{airlock_request_id}/exit-statistics", status_code=status_code.HTTP_200_OK,
                               response_model=AirlockRequestWithAllowedUserActions, name=strings.API_CHECK_STATISTICS_STATEMENTS,
                               dependencies=[Depends(get_current_workspace_owner_or_researcher_user), Depends(get_workspace_by_id_from_path)])
async def exit_and_reject_statistics(airlock_request=Depends(get_airlock_request_by_id_from_path),
                          airlock_request_repo=Depends(get_repository(AirlockRequestRepository)),
                          user=Depends(get_current_workspace_owner_or_researcher_user_or_airlock_manager)) -> AirlockRequestWithAllowedUserActions:
    if airlock_request.type == AirlockRequestType.Import:
        raise HTTPException(status_code=status_code.HTTP_400_BAD_REQUEST,
                            detail="Endpoint not available for Import Airlock requests.")
    try:
        rejected_airlock_request = await exit_and_reject_statistics_airlock_request(airlock_request, airlock_request_repo, user)
        return AirlockRequestWithAllowedUserActions(airlockRequest=rejected_airlock_request)
    except (ValidationError, ValueError) as e:
        logging.exception("Failed exiting and auto-reject export request.")
        raise HTTPException(status_code=status_code.HTTP_400_BAD_REQUEST, detail=str(e))
