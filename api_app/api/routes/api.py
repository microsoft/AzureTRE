...
workspace_tags_metadata = [
    {"name": "workspaces",
        "description": " **Workspace Owners and Researchers** can view their own workspaces"},
    ...
    {"name": "shared services",
        "description": "**TRE administratiors** administer shared services"}
]
...
core_router.include_router(
    workspace_templates.workspace_templates_admin_router, tags=["workspace templates"])
core_router.include_router(workspace_service_templates.workspace_service_templates_core_router, tags=[
                           "workspace service templates"])
core_router.include_router(user_resource_templates.user_resource_templates_core_router, tags=[
                           "user resource templates"])
core_router.include_router(shared_service_templates.shared_service_templates_core_router, tags=[
                           "shared service templates"])
core_router.include_router(
    shared_services.shared_services_router, tags=["shared services"])
core_router.include_router(operations.operations_router, tags=["operations"])
core_router.include_router(
    workspaces.workspaces_core_router, tags=["workspaces"])
core_router.include_router(
    workspaces.workspaces_shared_router, tags=["workspaces"])
core_router.include_router(
    migrations.migrations_core_router, tags=["migrations"])
...
core_router.include_router(
    workspace_users.workspaces_users_shared_router, tags=["users"])

if config.USER_MANAGEMENT_ENABLED:
    core_router.include_router(
        workspace_users.workspaces_users_admin_router, tags=["users"])
    core_router.include_router(
        workspace_users.workspaces_users_admin_router, tags=["assignable-users"])
    core_router.include_router(
        workspace_users.workspaces_users_admin_router, tags=["roles"])

core_swagger_router = APIRouter()
swagger_disabled_router = APIRouter()

openapi_definitions: DefaultDict[str, Optional[Dict[str, Any]]] = defaultdict(
    lambda: None)
...
async def get_disabled_swagger():
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                        detail=strings.SWAGGER_DISABLED)
...
workspace_router.include_router(
    workspaces.workspaces_shared_router, tags=["workspaces"])
workspace_router.include_router(
    workspaces.workspace_services_workspace_router, tags=["workspace services"])
workspace_router.include_router(
    workspaces.user_resources_workspace_router, tags=["user resources"])
workspace_router.include_router(
    costs.costs_workspace_router, tags=["workspace costs"])
workspace_router.include_router(
    airlock.airlock_workspace_router, tags=["airlock"])
