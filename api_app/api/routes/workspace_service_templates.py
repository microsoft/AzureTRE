...
async def get_workspace_service_templates(template_repo=Depends(Database().get_repository(ResourceTemplateRepository))) -> ResourceTemplateInformationInList:
...
async def get_workspace_service_template(service_template_name: str, is_update: bool = False, version: Optional[str] = None, template_repo=Depends(Database().get_repository(ResourceTemplateRepository))) -> WorkspaceServiceTemplateInResponse:
...
async def register_workspace_service_template(template_input: WorkspaceServiceTemplateInCreate, template_repo=Depends(Database().get_repository(ResourceTemplateRepository))) -> ResourceTemplateInResponse:
    try:
