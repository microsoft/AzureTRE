...
async def send_resource_request_message(resource: Resource, operations_repo: OperationRepository, action: RequestAction = RequestAction.Install) -> Operation:
    ...
    if action == RequestAction.Install:
        ...
        operation = operations_repo.create_operation_item(resource_id=resource.id, status=Status.Deleting, message=strings.RESOURCE_STATUS_DELETING, resource_path=resource.resourcePath)
    else:
        operation = operations_repo.create_operation_item(resource_id=resource.id, status=Status.InvokingAction, message=strings.RESOURCE_ACTION_STATUS_INVOKING, resource_path=resource.resourcePath)
