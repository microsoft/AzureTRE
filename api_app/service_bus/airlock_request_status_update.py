...
from api.routes.airlock.resource_helpers import update_and_publish_event_airlock_request
...
async def update_status_in_database(airlock_request_repo: AirlockRequestRepository, workspace_repo: WorkspaceRepository, step_result_message: StepResultStatusUpdateMessage):
    ...
    try:
        ...
        if airlock_request.status == current_status:
            ...
            await update_and_publish_event_airlock_request(airlock_request=airlock_request, airlock_request_repo=airlock_request_repo, user=airlock_request.user, new_status=new_status, workspace=workspace, request_files=request_files, status_message=status_message)
            result = True
        else:
