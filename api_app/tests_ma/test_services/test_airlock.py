...
@pytest.mark.asyncio
@patch("services.airlock.delete_review_user_resource")
async def test_delete_all_review_user_resources_retries_on_lease_contention(mock_delete_review):
    ...
    user_resource_repo = AsyncMock()
    user_resource_repo.get_user_resource_by_id.return_value = AsyncMock()

    # First attempt raises 409 Conflict (lease contention), second succeeds
    mock_op = MagicMock()
    mock_op.id = "op-1"
    mock_delete_review.side_effect = [
        HTTPException(status_code=409, detail="Workspace has an active operation"),
        mock_op,
    ]
