...
def test_substitution_for_primary_resource_parents(primary_resource, resource_ws_parent, resource_ws_svc_parent):
    ...
    # ws parent
    # single array val
    val_to_sub = "{{ parent_workspace.resource.properties.address_prefix }}"
    val = substitute_value(val_to_sub, None, parent_ws_resource_dict, None)
    assert val == ["172.1.1.1", "192.168.1.1"]

    # array val to inject, with text. Text will be dropped.
    val_to_sub = "{{ parent_workspace.resource.properties.fqdn }} - this text will be removed because fqdn is a list and shouldn't be concatenated into a string"
    val = substitute_value(val_to_sub, None, parent_ws_resource_dict, None)
    assert val == ["*pypi.org", "security.ubuntu.com"]

    # single string val, with text. Will be concatenated into text.
    val_to_sub = "I think {{ parent_workspace.resource.templateName }} is the best template!"
    val = substitute_value(val_to_sub, None, parent_ws_resource_dict, None)
    assert val == "I think ws template name is the best template!"

    # multiple string vals, with text. Will be concatenated.
    val_to_sub = "I think {{ parent_workspace.resource.templateName }} is the best template, and {{ parent_workspace.resource.templateVersion }} is a good version!"
    val = substitute_value(val_to_sub, None, parent_ws_resource_dict, None)
    assert val == "I think ws template name is the best template, and 8 is a good version!"

    # ws svc parent
    # single array val
    val_to_sub = "{{ parent_workspace_service.resource.properties.address_prefix }}"
    val = substitute_value(val_to_sub, None, None, parent_ws_svc_resource_dict)
    assert val == ["172.2.2.2", "192.168.2.2"]

    # array val to inject, with text. Text will be dropped.
    val_to_sub = "{{ parent_workspace_service.resource.properties.fqdn }} - this text will be removed because fqdn is a list and shouldn't be concatenated into a string"
    val = substitute_value(val_to_sub, None, None, parent_ws_svc_resource_dict)
    assert val == ["*pypi.org", "files.pythonhosted.org"]

    # single string val, with text. Will be concatenated into text.
    val_to_sub = "I think {{ parent_workspace_service.resource.templateName }} is the best template!"
    val = substitute_value(val_to_sub, None, None, parent_ws_svc_resource_dict)
    assert val == "I think svc template name is the best template!"

    # multiple string vals, with text. Will be concatenated.
    val_to_sub = "I think {{ parent_workspace_service.resource.templateName }} is the best template, and {{ parent_workspace_service.resource.templateVersion }} is a good version!"
    val = substitute_value(val_to_sub, None, None, parent_ws_svc_resource_dict)
    ...
    val_to_sub = "I am the primary resource ( a user resource - {{ resource.properties.display_name }}), my workspace parent is {{ parent_workspace_service.resource.properties.display_name }} and my parent workspace is {{ parent_workspace.resource.properties.display_name }}"
