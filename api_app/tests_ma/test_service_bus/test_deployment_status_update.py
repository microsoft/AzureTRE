...
from service_bus.deployment_status_update import DeploymentStatusUpdate
...
async def test_receiving_bad_json_logs_error(app, logging_mock, payload):
    ...
    status_updater = DeploymentStatusUpdate(app)
...
async def test_when_updating_and_state_store_exception(app, sb_client, logging_mock, repo, operation_repo):
    ...
    sb_client().get_queue_receiver = AsyncMock(return_value=[service_bus_received_message_mock])
...
@patch('service_bus.deployment_status_update.ResourceRepository')
@patch('logging.error')
@patch('service_bus.deployment_status_update.AutoLockRenewer')
@patch('service_bus.deployment_status_update.ServiceBusClient')
@patch('fastapi.FastAPI')
async def test_properties_dont_change_with_no_outputs(app, sb_client, renewer, logging_mock, repo, operations_repo):
