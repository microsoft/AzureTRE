...
@patch('service_bus.deployment_status_updater.send_deployment_message')
async def test_redelivered_main_status_does_not_rerun_completed_cleanup_or_fail_on_reallocated_address(send_deployment_message_mock):
    workspace_service_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    new_active_service_id = "9999c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    address_space = "10.1.0.0/22"
    ...
    new_active_service_mock = MagicMock()
    new_active_service_mock.id = new_active_service_id
    new_active_service_mock.properties = {"address_space": address_space}
...
@patch('service_bus.deployment_status_updater.send_deployment_message')
async def test_redelivered_main_status_skips_enqueue_when_cleanup_in_progress(send_deployment_message_mock):
    workspace_service_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    address_space = "10.1.0.0/22"
...
async def test_redelivered_main_status_skips_when_cleanup_failed_terminally(send_deployment_message_mock):
    ...
    step2 = OperationStep(
        ...
        sourceTemplateResourceId=parent_workspace_id,
        status=Status.UpdatingFailed,
        message="Cleanup failed terminally"
    )
    operation = Operation(
        id=OPERATION_ID,
        resourceId=workspace_service_id,
...
async def test_cleanup_success_persists_workspace_and_primary_resource_before_operation_deleted():
    ...
    async def tracking_op_update(op):
        call_order.append(("operations_repo.update_item", op.status))

    async def tracking_resource_update(res):
        call_order.append(("resource_repo.update_item", getattr(res, "id", None), res.deploymentStatus))

    async def tracking_resource_dict_update(res_dict):
        call_order.append(("resource_repo.update_item_dict", res_dict.get("id")))

    status_updater.operations_repo.update_item = AsyncMock(side_effect=tracking_op_update)
    status_updater.resource_repo = AsyncMock()
    status_updater.resource_repo.get_resource_by_id = AsyncMock(side_effect=mock_get_resource_by_id)
    status_updater.resource_repo.get_resource_dict_by_id.return_value = {
        "id": parent_workspace_id,
        "resourceType": ResourceType.Workspace,
        "properties": {"address_spaces": [address_space]}
    }
    status_updater.resource_repo.update_item = AsyncMock(side_effect=tracking_resource_update)
    status_updater.resource_repo.update_item_dict = AsyncMock(side_effect=tracking_resource_dict_update)
    status_updater.workspace_repo = AsyncMock()
...
async def test_merge_operation_steps_appends_missing_steps_and_recomputes_indices():
    status_updater = DeploymentStatusUpdater()
    step1 = OperationStep(
        id="step-1",
        stepTitle="Step 1",
    ...
    in_memory_op = Operation(
        id=OPERATION_ID,
        resourceId="res-1",
        resourcePath="/workspaces/res-ws/workspace-services/res-1",
        action=RequestAction.UnInstall,
        steps=[step1, step2],
    )
    fresh_step1 = copy.deepcopy(step1)
    fresh_op = Operation(
        id=OPERATION_ID,
        resourceId="res-1",
        resourcePath="/workspaces/res-ws/workspace-services/res-1",
    ...
    step_to_update, is_last_step, current_step_index = status_updater._merge_operation_steps(
        in_memory_op=in_memory_op,
        fresh_op=fresh_op,
        step_id_to_update="step-1",
        step_status=Status.Deleted,
        step_message="Uninstall done",
    )

    assert len(fresh_op.steps) == 2
    assert step_to_update.id == "step-1"
    assert step_to_update.status == Status.Deleted
    assert is_last_step is False
    assert current_step_index == 0
    assert fresh_op.steps[1].id == "step-2"


@patch('service_bus.deployment_status_updater.send_deployment_message')
async def test_etag_conflict_retry_merges_legacy_synthetic_cleanup_step(send_deployment_message_mock):
    workspace_service_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    address_space = "10.1.0.0/22"
    ...
    # Legacy operation with only 1 step initially in Cosmos DB
    legacy_op = Operation(
        id=OPERATION_ID,
        resourceId=workspace_service_id,
        resourcePath=f"/workspaces/{parent_workspace_id}/workspace-services/{workspace_service_id}",
    ...
    status_updater.operations_repo.get_operation_by_id.side_effect = mock_get_op
    ...
    async def mock_op_update(op):
        nonlocal attempt_count
        attempt_count += 1
        saved_operations.append(copy.deepcopy(op))
        if attempt_count == 1:
            raise CosmosAccessConditionFailedError()
        return op
...
async def test_cleanup_post_write_failure_on_final_delivery_dead_letters_when_terminal_persist_fails():
    workspace_service_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"
...
async def test_cleanup_post_write_failure_on_final_delivery_dead_letters_when_workspace_resource_write_fails():
    workspace_service_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"
...
async def test_get_address_cleanup_details_returns_none_if_resource_type_not_workspace_service():
    user_resource_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"

    step = OperationStep(
        id="step-1",
        stepTitle="Uninstall user resource",
    ...
    operation = Operation(
        id=OPERATION_ID,
        resourceId=user_resource_id,
        resourcePath=f"/workspaces/{parent_workspace_id}/user-resources/{user_resource_id}",
        resourceVersion=0,
        action=RequestAction.UnInstall,
        status=Status.PipelineRunning,
        steps=[step],
    ...
    status_updater.resource_repo.get_resource_dict_by_id.return_value = {
        ...
        "resourceType": ResourceType.UserResource,
        "workspaceId": parent_workspace_id,
        "properties": {"address_space": "10.0.1.0/24"},
    }

    details = await status_updater._get_address_cleanup_details(operation)
    assert details is None


async def test_etag_conflict_on_earlier_step_reconciles_and_skips_when_fresh_op_already_terminal():
    workspace_service_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    ...
    in_memory_op = Operation(
        ...
        action=RequestAction.UnInstall,
        status=Status.PipelineRunning,
        steps=[copy.deepcopy(step1), copy.deepcopy(step2)],
    )

    terminal_step1 = copy.deepcopy(step1)
    terminal_step1.status = Status.Deleted
    terminal_step2 = copy.deepcopy(step2)
    terminal_step2.status = Status.Updated
    terminal_step2.message = strings.ADDRESS_SPACE_CLEANUP_SUCCESS
    fresh_terminal_op = Operation(
        id=OPERATION_ID,
        resourceId=workspace_service_id,
        resourcePath=f"/workspaces/{parent_workspace_id}/workspace-services/{workspace_service_id}",
        resourceVersion=1,
        action=RequestAction.UnInstall,
        status=Status.Deleted,
        steps=[terminal_step1, terminal_step2],
    )
    ...
    status_updater = DeploymentStatusUpdater()
    status_updater.operations_repo = AsyncMock()
    status_updater.operations_repo.get_operation_by_id.side_effect = [
        in_memory_op,
        fresh_terminal_op,
    ]
    status_updater.operations_repo.update_item.side_effect = CosmosAccessConditionFailedError()
...
@patch('service_bus.deployment_status_updater.send_deployment_message')
async def test_etag_conflict_after_cleanup_dispatch_skips_when_fresh_op_already_terminal(send_deployment_message_mock):
    workspace_service_id = "59b5c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    parent_workspace_id = "1111c8e7-5c42-4fcb-a7fd-294cfc27aa76"
    ...
    fresh_terminal_op = Operation(
        ...
        action=RequestAction.UnInstall,
        status=Status.Deleted,
        steps=[copy.deepcopy(step1), terminal_step2],
    )
    ...
    def mock_get_op(op_id):
        nonlocal call_count
        call_count += 1
        return in_memory_op if call_count == 1 else fresh_terminal_op

    status_updater.operations_repo.get_operation_by_id.side_effect = mock_get_op

    op_update_count = 0

    async def mock_update_op(op):
        nonlocal op_update_count
        op_update_count += 1
        if op_update_count > 1:
            raise CosmosAccessConditionFailedError()
        return op

    status_updater.operations_repo.update_item.side_effect = mock_update_op
