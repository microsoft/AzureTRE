...
SHARED_SERVICE_TEMPLATE_NAME = 'test-shared-service'
...
def sample_shared_service(shared_service_id=SHARED_SERVICE_TEMPLATE_NAME):
...
class TestSharedServiceRoutesThatRequireAdminRights:
    ...
    async def test_get_shared_service_succeeds(self, get_shared_service_mock, app, client):
        shared_service = sample_shared_service()
        get_shared_service_mock.return_value = shared_service

        response = await client.get(
            app.url_path_for(strings.API_GET_SHARED_SERVICE_BY_ID, shared_service_id=SHARED_SERVICE_TEMPLATE_NAME))
    ...
    async def test_get_shared_service_raises_404_if_not_found(self, get_shared_service_mock, _,
                                                              app, client):
        get_shared_service_mock.return_value = sample_shared_service(SHARED_SERVICE_TEMPLATE_NAME)
    ...
    async def test_patch_shared_service_returns_404_if_does_not_exist(self, _, app, client):
        response = await client.patch(app.url_path_for(strings.API_UPDATE_SHARED_SERVICE, shared_service_id=SHARED_SERVICE_TEMPLATE_NAME), json='{"enabled": true}')
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    async def test_patch_shared_service_returns_409_if_bad_etag(self, _, __, app, client):
        ...
        response = await client.patch(app.url_path_for(strings.API_UPDATE_SHARED_SERVICE, shared_service_id=SHARED_SERVICE_TEMPLATE_NAME), json=shared_service_patch, headers={"etag": etag})
    ...
    @patch("api.dependencies.shared_services.SharedServiceRepository.get_shared_service_by_id", return_value=sample_shared_service(SHARED_SERVICE_TEMPLATE_NAME))
    ...
    @patch("api.routes.shared_services.send_resource_request_message", return_value=sample_resource_operation(resource_id=SHARED_SERVICE_TEMPLATE_NAME, operation_id=OPERATION_ID))
