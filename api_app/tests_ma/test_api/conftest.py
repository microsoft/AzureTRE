...
def get_required_roles(endpoint):
    ...
    dependencies = list(filter(lambda x: getattr(x, 'dependency', None) is not None and hasattr(x.dependency, 'require_one_of_roles'), defaults))
    if dependencies:
        return dependencies[0].dependency.require_one_of_roles
    # New-style deps: check for _role_names attribute on the closure
    dependencies = list(filter(lambda x: getattr(x, 'dependency', None) is not None and hasattr(x.dependency, '_role_names'), defaults))
    if dependencies:
