...
class SharedServiceRepository(ResourceRepository):
    ...
    @staticmethod
    def not_deleted_shared_service_with_template_name_query(template_name: str):
        return f'SELECT * FROM c WHERE {IS_NOT_DELETED_CLAUSE} AND c.resourceType = "{ResourceType.SharedService}" AND c.templateName = "{template_name}"'
    ...
    def create_shared_service_item(self, shared_service_input: SharedServiceTemplateInCreate) -> Tuple[SharedService, ResourceTemplate]:
        ...
        existing_shared_services = self.query(self.not_deleted_shared_service_with_template_name_query(shared_service_input.templateName))
