...
def get_storage_account_name_for_request(
    ...
    if airlock_version >= 2:
        ...
        if request_type == constants.IMPORT_TYPE:
            ...
            if status in [AirlockRequestStatus.Rejected, AirlockRequestStatus.RejectionInProgress,
                          AirlockRequestStatus.Blocked, AirlockRequestStatus.BlockingInProgress]:
                # These are in core storage
                return constants.STORAGE_ACCOUNT_NAME_AIRLOCK_CORE.format(tre_id)
        elif request_type == constants.EXPORT_TYPE:
            if status in [AirlockRequestStatus.Approved, AirlockRequestStatus.ApprovalInProgress]:
        ...
        raise ValueError(f"Unsupported airlock request status '{status}' for consolidated storage account resolution")
    ...
    if request_type == constants.IMPORT_TYPE:
        ...
        if status in [AirlockRequestStatus.Submitted, AirlockRequestStatus.InReview]:
        ...
        if status in [AirlockRequestStatus.Blocked, AirlockRequestStatus.BlockingInProgress]:
    ...
    raise ValueError(f"Unsupported airlock request status '{status}' for legacy storage account resolution")
