locals {
  ...
  legacy_eventgrid_system_topic_resources = var.enable_legacy_airlock ? [
    try(azurerm_eventgrid_system_topic.import_inprogress_blob_created[0], null),
    try(azurerm_eventgrid_system_topic.import_rejected_blob_created[0], null),
    try(azurerm_eventgrid_system_topic.import_blocked_blob_created[0], null),
    try(azurerm_eventgrid_system_topic.export_approved_blob_created[0], null),
  ] : []

  legacy_eventgrid_system_topics = {
    for topic in local.legacy_eventgrid_system_topic_resources : topic.name => topic.id
    if topic != null
  }
